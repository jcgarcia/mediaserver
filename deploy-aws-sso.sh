...
# AWS Media Server Deployment Script - SSO Version
# Optimized for AWS SSO/Identity Center environments

set -e

echo "🚀 Starting AWS Media Server Deployment (SSO Version)..."
...
# Step 2: Create IAM roles for ECS (modified for SSO environment)
...
aws iam attach-role-policy --role-name ecsTaskExecutionRole --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy 2>/dev/null || echo "Policy already attached"
...
aws logs create-log-group --log-group-name /ecs/mediaserver --region ${REGION} 2>/dev/null || echo "Log group already exists"
...
echo ""
echo "🌐 To get public IP addresses:"
...
echo "🔐 JWT Secret stored in Parameter Store: /mediaserver/jwt-secret"
